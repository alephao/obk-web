...
describe 'Event', :type => :request do
  ...
  describe 'PUT /api/events/:id/resign' do
    describe 'with authenticated volunteer' do
      before do
        future_lunch_at_noon.volunteers << volunteer
      end

      it 'should decrement the number of volunteers' do
        expect do
          put "/api/events/#{future_lunch_at_noon.id}/resign", headers: @auth_headers
        end.to change { future_lunch_at_noon.volunteers .count }.by(-1)
      end
    ...
    describe 'with an event not joined' do
      let(:future_lunch_at_noon) { create(:future_lunch_at_noon) }
      ...
      it 'should not count the volunteer' do
        future_lunch_at_noon.reload
        expect(future_lunch_at_noon.volunteers.size).to eq(0)
      end
    end

    describe 'with no authentication' do
      before do
        put "/api/events/#{future_lunch_at_noon.id}/resign"
      end
    ...
    describe 'with a past event' do
      ...
      before do
        past_event.volunteers << volunteer
        put "/api/events/#{past_event.id}/resign", headers: @auth_headers
      ...
      it 'should contain one error' do
        ...
        expect(@response_json['errors'].first).to include('expired')
      end

      it 'should not remove the volunteer' do
        past_event.reload
        expect(past_event.volunteers.size).to eq(1)
      end
    end
  end
end